...
public class ArrayBasedQueue<T> implements Queue<T> {
	...
	return (this.back() + 1) % values.length == this.front;
    } // isFull()
     ...
     */
    int back() {
	return (this.front + this.size) % values.length;
    } // back()
...
class ArrayBasedQueueIterator<T> implements Iterator<T> {
	...
	this.pos = (this.pos + 1) % this.q.values.length;
	return q.values[this.pos];
    } // next()

    @Override
    public boolean hasNext() {
	return ((this.pos + 1) % this.q.values.length != this.q.back());
    } // hasNext()
